...
void Wolf3D::addCube(std::vector<glm::vec3>& vertexPositions, std::vector<glm::vec4>& textureCoordinates, int x, int z, int type){
    ...
    auto lbf = glm::vec3(x-0.5,-0.5,z+0.5);
    auto rbf = glm::vec3(x+0.5,-0.5,z+0.5);
    auto ltf = glm::vec3(x-0.5,0.5,z+0.5);
    auto rtf = glm::vec3(x+0.5,0.5,z+0.5);
    auto lbb = glm::vec3(x-0.5,-0.5,z-0.5);
    auto rbb = glm::vec3(x+0.5,-0.5,z-0.5);
    auto ltb = glm::vec3(x-0.5,0.5,z-0.5);
    auto rtb = glm::vec3(x+0.5,0.5,z-0.5);
    ...
    glm::vec2 min = vec2((type * 2) % 16 * tileSizeWithBorder.x,(42 - type / 8)*tileSizeWithBorder.y) / textureSize;
    glm::vec2 max = min+tileSize / textureSize;
    glm::vec2 min2 = vec2(((type * 2) % 16 + 1) * tileSizeWithBorder.x, (42 - type / 8)*tileSizeWithBorder.y) / textureSize;
    glm::vec2 max2 = min2+tileSize / textureSize;
    textureCoordinates.insert(textureCoordinates.end(),{
            ...
            glm::vec4(min2.x,min2.y,0,0), glm::vec4(max2.x,min2.y,0,0), glm::vec4(min2.x,max2.y,0,0),
            glm::vec4(max2.x,max2.y,0,0), glm::vec4(min2.x,max2.y,0,0), glm::vec4(max2.x,min2.y,0,0),
            glm::vec4(min.x,min.y,0,0), glm::vec4(max.x,min.y,0,0), glm::vec4(min.x,max.y,0,0),
            glm::vec4(max.x,max.y,0,0), glm::vec4(min.x,max.y,0,0), glm::vec4(max.x,min.y,0,0),
            glm::vec4(min2.x,min2.y,0,0), glm::vec4(max2.x,min2.y,0,0), glm::vec4(min2.x,max2.y,0,0),
            glm::vec4(max2.x,max2.y,0,0), glm::vec4(min2.x,max2.y,0,0), glm::vec4(max2.x,min2.y,0,0)
